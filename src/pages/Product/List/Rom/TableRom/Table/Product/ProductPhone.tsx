...
import { deleteRom, getRoms } from "src/store/rom/romSlice";
...
export default function ProductRom({ product }: Props) {
  ...
  const editOptions = [
    {
      ...
      callback: () => {
        navigate(
          `${"/rom/detail/update"}/${generateNameId({
            name: product.name,
    ...
    },
    {
      ...
      callback: () => {
        const handleDelete = async () => {
          const res = await dispatch(deleteRom(product.id.toString()));
          unwrapResult(res);
          // const d = res?.payload;
