...
export default function ProductMicrophone({ product }: Props) {
  ...
  const editOptions = [
    {
      ...
      callback: () => {
        navigate(
          `${"/microphone/detail/update"}/${generateNameId({
            name: product.name,
      ...
      callback: () => {
        if (confirm("Bạn có muốn disable sản phẩm không?")) {
          const handleDelete = async () => {
            const res = await dispatch(deleteMicrophone(product.id.toString()));
            unwrapResult(res);
  ...
  return (
    <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <Link
        to={`${"/microphone/detail"}/${generateNameId({
          name: product.name,
          id: product.id.toString(),
        })}`}
      >
