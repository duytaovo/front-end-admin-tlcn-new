...
export default function ProductAdapter({ product }: Props) {
  ...
  const editOptions = [
    {
      ...
      callback: () => {
        navigate(
          `${"/computerPower/detail/update"}/${generateNameId({
            name: product.name,
        ...
        if (confirm("Bạn có muốn disable sản phẩm không?")) {
          const handleDelete = async () => {
            const res = await dispatch(
              deleteComputerPower(product.id.toString()),
            );
            unwrapResult(res);
  ...
  return (
    <div onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <Link
        to={`${"/computerPower/detail"}/${generateNameId({
          name: product.name,
          id: product.id.toString(),
        })}`}
      >
