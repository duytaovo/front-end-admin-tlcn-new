...
import { deleteRam, getRams } from "src/store/ram/ramSlice";
...
export default function ProductRam({ product }: Props) {
  ...
  const editOptions = [
    {
      ...
      callback: () => {
        navigate(
          `${"/ram/detail/update"}/${generateNameId({
            name: product.name,
    ...
    },
    {
      ...
      callback: () => {
        const handleDelete = async () => {
          const res = await dispatch(deleteRam(product.id.toString()));
          unwrapResult(res);
          // const d = res?.payload;
