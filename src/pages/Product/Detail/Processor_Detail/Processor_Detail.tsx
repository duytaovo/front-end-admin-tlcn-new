...
import { getDetailProcessor } from "src/store/processor/processorSlice";
...
export default function ProcessorDetail() {
  ...
  const currentImages = useMemo(
    () =>
      processorDetail?.productInfo?.lstProductImageUrl
        ? processorDetail?.productInfo?.lstProductImageUrl.slice(
            ...currentIndexImages,
          )
        : [],
    [processorDetail, currentIndexImages],
  );

  useEffect(() => {
    if (
      processorDetail &&
      processorDetail?.productInfo?.lstProductImageUrl?.length > 0
    ) {
      setActiveImage(processorDetail?.productInfo?.lstProductImageUrl[0]);
    }
  }, [processorDetail, activeImage]);
  ...
  const next = () => {
    if (
      ...
      processorDetail?.productInfo.lstProductImageUrl.length
    ) {
  ...
  if (!processorDetail) return null;

  return (
    <div className="bg-gray-200 py-6">
      <Helmet>
        <title>{processorDetail?.productInfo?.name}</title>
        <meta
          name="description"
          content={convert(processorDetail?.productInfo?.description, {
            limits: {
          ...
          <div className="grid grid-cols-12 gap-9">
            <div className="col-span-5">
              ...
              >
                <img
                  src={activeImage}
                  alt={processorDetail?.productInfo?.name}
            ...
            <div className="col-span-7">
              <h1 className="text-4xl font-medium uppercase">
                {processorDetail?.productInfo?.name}
              </h1>
              <div className="mt-8 flex items-center">
                <div className="flex items-center">
                  <span className="mr-1 border-b border-b-orange text-orange">
                    {processorDetail?.productInfo?.star}
                  </span>
                  <Rate
                    allowHalf
                    defaultValue={Number(
                      processorDetail?.productInfo?.totalReview,
                    )}
                    disabled
                  />
                  ;
                ...
                <div>
                  <span>
                    {formatNumberToSocialStyle(
                      Number(processorDetail?.productInfo?.totalReview),
                    )}
                  </span>
              ...
              <div className="space-x-3 mt-4 flex justify-start align-baseline">
                <Tag productData={processorDetail} />
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <div className="block space-y-2">
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>{processorDetail?.monitor}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Hệ điều hành :</h4>
              <h5>{processorDetail?.operatingSystem}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Camera chính :</h4>
              <h5>{processorDetail?.rearCamera}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Camera trước :</h4>
              <h5>{processorDetail?.frontCamera}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Chip :</h4>
              <h5>{processorDetail?.chip}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Sim :</h4>
              <h5>{processorDetail?.sim}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
            ...
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>{processorDetail?.charging}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Hỗ trợ mạng:</h4>
              <h5>{processorDetail?.networkSupport}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Phụ kiện:</h4>
              <h5>{processorDetail?.productInfo?.accessories}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Năm ra mắt:</h4>
              <h5>{processorDetail?.productInfo?.launchTime}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Thiết kế:</h4>
              <h5>{processorDetail?.productInfo?.design}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Khối lượng:</h4>
              <h5>{processorDetail?.productInfo?.mass}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Ram:</h4>
              <h5>
                {processorDetail?.productInfo?.lstProductTypeAndPrice[0]?.ram}
              </h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>
                {
                  processorDetail?.productInfo?.lstProductTypeAndPrice[0]
                    ?.storageCapacity
                }
              </h5>
            </div>
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(
                    processorDetail?.productInfo?.description,
                  ),
                }}
              />
            </div>
