...
import { getDetailRam } from "src/store/ram/ramSlice";

export default function RamDetail() {
  ...
  const currentImages = useMemo(
    () =>
      ramDetail?.productInfo?.lstProductImageUrl
        ? ramDetail?.productInfo?.lstProductImageUrl.slice(
            ...currentIndexImages,
          )
        : [],
    [ramDetail, currentIndexImages],
  );

  useEffect(() => {
    if (ramDetail && ramDetail?.productInfo?.lstProductImageUrl?.length > 0) {
      setActiveImage(ramDetail?.productInfo?.lstProductImageUrl[0]);
    }
  }, [ramDetail, activeImage]);
  ...
  const next = () => {
    if (
      currentIndexImages[1] < ramDetail?.productInfo.lstProductImageUrl.length
    ) {
  ...
  if (!ramDetail) return null;

  return (
    <div className="bg-gray-200 py-6">
      <Helmet>
        <title>{ramDetail?.productInfo?.name}</title>
        <meta
          name="description"
          content={convert(ramDetail?.productInfo?.description, {
            limits: {
          ...
          <div className="grid grid-cols-12 gap-9">
            <div className="col-span-5">
              ...
              >
                <img
                  src={activeImage}
                  alt={ramDetail?.productInfo?.name}
            ...
            <div className="col-span-7">
              <h1 className="text-4xl font-medium uppercase">
                {ramDetail?.productInfo?.name}
              </h1>
              <div className="mt-8 flex items-center">
                <div className="flex items-center">
                  <span className="mr-1 border-b border-b-orange text-orange">
                    {ramDetail?.productInfo?.star}
                  </span>
                  <Rate
                    allowHalf
                    defaultValue={Number(ramDetail?.productInfo?.totalReview)}
                    disabled
                  />
                  ;
                </div>
                ...
                <div>
                  <span>
                    {formatNumberToSocialStyle(
                      Number(ramDetail?.productInfo?.totalReview),
                    )}
                  </span>
        ...
        >
          <div className="block space-y-2">
            ...
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>{ramDetail?.charging}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Hỗ trợ mạng:</h4>
              <h5>{ramDetail?.networkSupport}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Phụ kiện:</h4>
              <h5>{ramDetail?.productInfo?.accessories}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Năm ra mắt:</h4>
              <h5>{ramDetail?.productInfo?.launchTime}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Thiết kế:</h4>
              <h5>{ramDetail?.productInfo?.design}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Khối lượng:</h4>
              <h5>{ramDetail?.productInfo?.mass}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Ram:</h4>
              <h5>{ramDetail?.productInfo?.lstProductTypeAndPrice[0]?.ram}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Bộ nhớ trong:</h4>
              <h5>
                {
                  ramDetail?.productInfo?.lstProductTypeAndPrice[0]
                    ?.storageCapacity
                }
              </h5>
            </div>
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(
                    ramDetail?.productInfo?.description,
                  ),
                }}
              />
            </div>
