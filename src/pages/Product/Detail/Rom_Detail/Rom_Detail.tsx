...
import { getDetailRom } from "src/store/rom/romSlice";
...
export default function RomDetail() {
  ...
  const currentImages = useMemo(
    () =>
      romDetail?.productInfo?.lstProductImageUrl
        ? romDetail?.productInfo?.lstProductImageUrl.slice(
            ...currentIndexImages,
          )
        : [],
    [romDetail, currentIndexImages],
  );

  useEffect(() => {
    if (romDetail && romDetail?.productInfo?.lstProductImageUrl?.length > 0) {
      setActiveImage(romDetail?.productInfo?.lstProductImageUrl[0]);
    }
  }, [romDetail, activeImage]);
  ...
  const next = () => {
    if (
      currentIndexImages[1] < romDetail?.productInfo.lstProductImageUrl.length
    ) {
  ...
  if (!romDetail) return null;

  return (
    <div className="bg-gray-200 py-6">
      <Helmet>
        <title>{romDetail?.productInfo?.name}</title>
        <meta
          name="description"
          content={convert(romDetail?.productInfo?.description, {
            limits: {
          ...
          <div className="grid grid-cols-12 gap-9">
            <div className="col-span-5">
              ...
              >
                <img
                  src={activeImage}
                  alt={romDetail?.productInfo?.name}
            ...
            <div className="col-span-7">
              <h1 className="text-4xl font-medium uppercase">
                {romDetail?.productInfo?.name}
              </h1>
              <div className="mt-8 flex items-center">
                <div className="flex items-center">
                  <span className="mr-1 border-b border-b-orange text-orange">
                    {romDetail?.productInfo?.star}
                  </span>
                  <Rate
                    allowHalf
                    defaultValue={Number(romDetail?.productInfo?.totalReview)}
                    disabled
                  />
                  ;
                </div>
                ...
                <div>
                  <span>
                    {formatNumberToSocialStyle(
                      Number(romDetail?.productInfo?.totalReview),
                    )}
                  </span>
        ...
        >
          <div className="block space-y-2">
            ...
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>{romDetail?.charging}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Hỗ trợ mạng:</h4>
              <h5>{romDetail?.networkSupport}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Phụ kiện:</h4>
              <h5>{romDetail?.productInfo?.accessories}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Năm ra mắt:</h4>
              <h5>{romDetail?.productInfo?.launchTime}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Thiết kế:</h4>
              <h5>{romDetail?.productInfo?.design}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Khối lượng:</h4>
              <h5>{romDetail?.productInfo?.mass}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Ram:</h4>
              <h5>{romDetail?.productInfo?.lstProductTypeAndPrice[0]?.ram}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Bộ nhớ trong:</h4>
              <h5>
                {
                  romDetail?.productInfo?.lstProductTypeAndPrice[0]
                    ?.storageCapacity
                }
              </h5>
            </div>
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(
                    romDetail?.productInfo?.description,
                  ),
                }}
              />
            </div>
