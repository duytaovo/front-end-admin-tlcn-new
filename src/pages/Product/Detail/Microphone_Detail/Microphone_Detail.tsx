...
export default function MicrophoneDetail() {
  ...
  const { microphoneDetail } = useAppSelector((state) => state.microphone);
  ...
  const currentImages = useMemo(
    () =>
      microphoneDetail?.productInfo?.lstProductImageUrl
        ? microphoneDetail?.productInfo?.lstProductImageUrl.slice(
            ...currentIndexImages,
          )
        : [],
    [microphoneDetail, currentIndexImages],
  );

  useEffect(() => {
    if (
      microphoneDetail &&
      microphoneDetail?.productInfo?.lstProductImageUrl?.length > 0
    ) {
      setActiveImage(microphoneDetail?.productInfo?.lstProductImageUrl[0]);
    }
  }, [microphoneDetail, activeImage]);

  useEffect(() => {
    dispatch(getDetailMicrophone(id));
  }, [id]);

  const next = () => {
    if (
      currentIndexImages[1] <
      microphoneDetail?.productInfo.lstProductImageUrl.length
    ) {
  ...
  if (!microphoneDetail) return null;

  return (
    <div className="bg-gray-200 py-6">
      <Helmet>
        <title>{microphoneDetail?.productInfo?.name}</title>
        <meta
          name="description"
          content={convert(microphoneDetail?.productInfo?.description, {
            limits: {
          ...
          <div className="grid grid-cols-12 gap-9">
            <div className="col-span-5">
              ...
              >
                <img
                  src={activeImage}
                  alt={microphoneDetail?.productInfo?.name}
            ...
            <div className="col-span-7">
              <h1 className="text-4xl font-medium uppercase">
                {microphoneDetail?.productInfo?.name}
              </h1>
              <div className="mt-8 flex items-center">
                <div className="flex items-center">
                  <span className="mr-1 border-b border-b-orange text-orange">
                    {microphoneDetail?.productInfo?.star}
                  </span>
                  <Rate
                    allowHalf
                    defaultValue={Number(
                      microphoneDetail?.productInfo?.totalReview,
                    )}
                    disabled
                  />
                  ;
                ...
                <div>
                  <span>
                    {formatNumberToSocialStyle(
                      Number(microphoneDetail?.productInfo?.totalReview),
                    )}
                  </span>
              ...
              <div className="space-x-3 mt-4 flex justify-start align-baseline">
                <Tag productData={microphoneDetail} />
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <div className="block space-y-2">
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>{microphoneDetail?.monitor}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Hệ điều hành :</h4>
              <h5>{microphoneDetail?.operatingSystem}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Camera chính :</h4>
              <h5>{microphoneDetail?.rearCamera}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Camera trước :</h4>
              <h5>{microphoneDetail?.frontCamera}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Chip :</h4>
              <h5>{microphoneDetail?.chip}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Sim :</h4>
              <h5>{microphoneDetail?.sim}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
            ...
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>{microphoneDetail?.charging}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Hỗ trợ mạng:</h4>
              <h5>{microphoneDetail?.networkSupport}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Phụ kiện:</h4>
              <h5>{microphoneDetail?.productInfo?.accessories}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Năm ra mắt:</h4>
              <h5>{microphoneDetail?.productInfo?.launchTime}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Thiết kế:</h4>
              <h5>{microphoneDetail?.productInfo?.design}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
              <h4 className="font-bold">Khối lượng:</h4>
              <h5>{microphoneDetail?.productInfo?.mass}</h5>
            </div>
            <div className="flex justify-start align-baseline space-x-4">
            ...
            <div className="flex justify-start align-baseline space-x-4">
              ...
              <h5>
                {
                  microphoneDetail?.productInfo?.lstProductTypeAndPrice[0]
                    ?.storageCapacity
                }
              </h5>
            </div>
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(
                    microphoneDetail?.productInfo?.description,
                  ),
                }}
              />
            </div>
